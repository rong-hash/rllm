...
# tilelang: flash-linear-attention refuses to run gated_delta_rule backward
# on Hopper GPUs with Triton >= 3.4 (known-incorrect results, see fla#640).
# It hard-raises RuntimeError asking us to install tilelang as replacement
# backend. Without this the GDN backward fails on K1 (H200).
pip install --upgrade \
...
# ── FLASH-ATTN EXPERIMENT ──────────────────────────────────────────────
# Pre-built flash-attn wheels (PyPI + pypi.msh.team) all fail ABI check
# against this image's torch 2.10+cu129.msh. This variant builds flash-attn
# from source so it links against the exact torch in this env.
# ~30-60 min build on first submission; cached to moonfs for reuse.
# Fails fast (exit 1) if build or smoke test fails.
...
# Tiny forward — catches ABI mismatches at runtime, not just import
B, N, H, D = 1, 16, 8, 64
...
assert out.shape == (B, N, H, D), f"unexpected shape {out.shape}"
print(f"flash_attn smoke test OK: out.shape={out.shape}")

mask = torch.ones(2, 10, dtype=torch.int32, device="cuda")
mask[1, 7:] = 0
