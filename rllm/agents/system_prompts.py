...
# Generic coding-agent scaffold — same tool definitions as SWE, no github-issue
# / repository / "minimal changes to non-tests files" framing. Use for tasks
# where the agent writes new code from scratch (e.g. CodeV-R1 Verilog gen).
CODING_SYSTEM_PROMPT = """You are a coding agent with access to a bash environment. You are given a programming task to complete. Read the task carefully, explore the workspace when needed, write and edit files as required, and run commands to verify your work before submitting.
...
# Passthrough user prompt — the task's instruction is used verbatim.
CODING_USER_PROMPT = """{problem_statement}"""
